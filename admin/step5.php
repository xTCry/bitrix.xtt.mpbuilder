...
echo BeginNote() .
    GetMessage("BITRIX_MPBUILDER_V_ARHIV_POPADUT_FAYL") . ' install/version.php. ' . GetMessage("BITRIX_MPBUILDER_OBNOVLENIE_NEOBHODIM") .
...
if ($_REQUEST['action'] == 'delete' && $module_id && check_bitrix_sessid()) {
    ...
    if ($bCustomNameSpace = array_key_exists('NAMESPACE', $_REQUEST))
        COption::SetOptionString($MODULE_ID, 'NAMESPACE', $NAMESPACE = str_replace(['/', '\\', ' '], '', $_REQUEST['NAMESPACE']));
    ...
    if (!$strError) {
        ...
        if (!$strError && $_REQUEST['components']) {
            ...
            if ($bCustomNameSpace) {
                ...
                while (false !== $item = readdir($dir)) {
                    ...
                    $arTmp = [];
                    $type = 'bitrix';
                    if (is_dir($f = $_SERVER['DOCUMENT_ROOT'] . '/local/components/' . $NAMESPACE . '/' . $item)) {
                        $arTmp = BuilderGetFiles($f, [], true);
                        $type = 'local';
                    } elseif (is_dir($f = $_SERVER['DOCUMENT_ROOT'] . '/bitrix/components/' . $NAMESPACE . '/' . $item)) {
                        $arTmp = BuilderGetFiles($f, [], true);
                    }
                    foreach ($arTmp as $file) {
                        $ar['/' . $NAMESPACE . '/' . $item . $file] = $type;
                    }
                }
                closedir($dir);
            } else {
                ...
                while (false !== $item = readdir($dir)) {
                    ...
                    while (false !== $item0 = readdir($dir0)) {
                        ...
                        $type = 'bitrix';
                        if (is_dir($_SERVER['DOCUMENT_ROOT'] . '/local/components/' . $item . '/' . $item0)) {
                            $arTmp = BuilderGetFiles($_SERVER['DOCUMENT_ROOT'] . '/local/components/' . $item . '/' . $item0, [], true);
                            $type = 'local';
                        } else {
                            $arTmp = BuilderGetFiles($_SERVER['DOCUMENT_ROOT'] . '/bitrix/components/' . $item . '/' . $item0, [], true);
                        }

                        foreach ($arTmp as $file) {
                            $ar['/' . $item . '/' . $item0 . $file] = $type;
                        }
                    }
                    closedir($dir0);
                }
            ...
            foreach ($ar as $file => $type) {
                $from = $_SERVER['DOCUMENT_ROOT'] . '/' . $type . '/components' . $file;
        ...
        $tar = new CTarBuilder;
        $tar->path = $tmp_dir;
        if (!$tar->openWrite($f = $tmp_dir . '/' . $tarFile)) {
            $strError .= GetMessage("BITRIX_MPBUILDER_NE_UDALOSQ_OTKRYTQ_F") . $f . '<br>';
        } else {
            $ar = BuilderGetFiles($m_dir, [], true);
...
?>
<form action="<?= $APPLICATION->GetCurPage() ?>?lang=<?= LANG ?>" method="POST" enctype="multipart/form-data" name="builder_form">
    <?= bitrix_sessid_post() ?>
    <?
    $editTab->Begin();
    ...
    <? if ($module_id) { ?>
        <tr>
            ...
            <td>
                <input name="version" value="<?= ($v = $_REQUEST['version'] ? htmlspecialcharsbx($_REQUEST['version']) : VersionUp($arModuleVersion['VERSION'])) ?>">
                <label><input type="checkbox" name="store_version" <?= $_REQUEST['store_version'] ? 'checked' : '' ?>> <?= GetMessage("BITRIX_MPBUILDER_OBNOVITQ_DATU_I_VERS") ?>
                </label>
                <?
        ...
        <? if ($bCustomNameSpace) {   ?>
            <tr>
                ...
                <td>/bitrix/components/<input name="NAMESPACE" id="NAMESPACE" value="<?= htmlspecialcharsbx($NAMESPACE) ?>" <?= $_REQUEST['components'] ? '' : 'disabled' ?>>
                </td>
            </tr>
            <tr>
                <td></td>
                <td class="smalltext"><?= GetMessage("BITRIX_MPBUILDER_KOMPONENTY_MODULA_LE") ?>
                    ...
                    <i>updater.php</i> <?= GetMessage("BITRIX_MPBUILDER_PRAVILQNO_KOPIRUET_K") ?>
                </td>
            </tr>
        <?
        }
        ...
        <tr>
            ...
            <td>
                ...
                ?>
                <textarea name="description" style="width:100%" rows="10"><?= htmlspecialcharsbx($description) ?></textarea>
            </td>
        </tr>
        <tr>
            <td></td>
        ...
        <tr>
            ...
            <td>
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST')
                    ...
                    $updater = file_get_contents($_SERVER['DOCUMENT_ROOT'] . '/' . $xttMPBRootModuleDir . '/modules/' . $MODULE_ID . '/samples/_updater.php');
                    $updater = str_replace('{MODULE_ID}', $module_id, $updater);
                    $updater = str_replace('{NAMESPACE}', $bCustomNameSpace ? $NAMESPACE : '', $updater);
                }
                ?>
                <textarea name="updater" style="width:100%" rows="10"><?= htmlspecialcharsbx($updater) ?></textarea>
            </td>
        </tr>
    <?
    }
