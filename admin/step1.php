...
if ($_POST['save'] && check_bitrix_sessid()) {
	...
	$bToLocal = $_REQUEST['to_local'];
	$module = $partner_code . '.' . $module_id;

	if (!$strError) {
		$m_dir = $_SERVER['DOCUMENT_ROOT'] . '/' . ($bToLocal ? 'local' : 'bitrix') . '/modules/' . $module;
		$arStructure = array(
			$m_dir,
			$m_dir . '/admin',
	...
	if (!$strError) {
		...
		($str = file_get_contents($f = $_SERVER['DOCUMENT_ROOT'] . '/' . $xttMPBRootModuleDir . '/modules/' . $MODULE_ID . '/samples/install/index.php')) || die(GetMessage("BITRIX_MPBUILDER_FAYL_NE_NAYDEN") . $f);
		...
		($str1 = file_get_contents($f = $_SERVER['DOCUMENT_ROOT'] . '/' . $xttMPBRootModuleDir . '/modules/' . $MODULE_ID . '/samples/include.php')) || die(GetMessage("BITRIX_MPBUILDER_FAYL_NE_NAYDEN") . $f);
		...
		if (!$strError && is_array($_REQUEST['component'])) {
			foreach ($_REQUEST['component'] as $c) {
				if (!$c)
					continue;
				if (!CopyDirFiles($_SERVER['DOCUMENT_ROOT'] . $c, $m_dir . '/install' . str_replace(['local', BX_ROOT], ['', ''], $c), $bForce, true))
	...
	if ($strError)
		...
		$_SESSION['mpbuilder']['module_id'] = $module;
		$_SESSION['mpbuilder']['rootDir'] = $bToLocal ? 'local' : 'bitrix';
		LocalRedirect('/bitrix/admin/' . $MODULE_ID . '_step2.php?module_id=' . $module . '&lang=' . LANGUAGE_ID);
	}
}
?>
<form action="<?= $APPLICATION->GetCurPage() ?>?lang=<?= LANG ?>" method="POST" enctype="multipart/form-data">
	...
	<tr>
		...
		<td><input name="partner_name" value="<?= htmlspecialcharsbx(COption::GetOptionString($MODULE_ID, 'partner_name', '')) ?>"></td>
	</tr>
	<tr>
		<td><?= GetMessage("BITRIX_MPBUILDER_ADRES_VASEGO_SAYTA") ?></td>
		<td><input name="partner_uri" value="<?= htmlspecialcharsbx(COption::GetOptionString($MODULE_ID, 'partner_uri', 'http://www.mysite.ru')) ?>"></td>
	</tr>
	<tr>
		<td><?= GetMessage("BITRIX_MPBUILDER_KOD_PARTNERA") ?></td>
		<td><input name="partner_code" value="<?= htmlspecialcharsbx(COption::GetOptionString($MODULE_ID, 'partner_code', '')) ?>"></td>
	</tr>
	<tr>
		<td></td>
	...
	<tr>
		<td><?= GetMessage("BITRIX_MPBUILDER_TO_LOCAL") ?></td>
		<td><input type="checkbox" name="to_local" <?= (!isset($_REQUEST['to_local']) || $_REQUEST['to_local']) ? 'checked' : '' ?>></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><input name="module_id" value="<?= htmlspecialcharsbx($module_id) ?>"></td>
	</tr>
	<tr>
		<td></td>
	...
	<tr>
		...
		<td><input name="module_name" value="<?= htmlspecialcharsbx($module_name) ?>"></td>
	</tr>
	<tr>
		<td valign="top"><?= GetMessage("BITRIX_MPBUILDER_OPISANIE_MODULA") ?></td>
		<td><textarea name="module_desc" cols="40" rows="3"><?= htmlspecialcharsbx($module_desc) ?></textarea></td>
	</tr>
	<tr class="heading">
		<td colspan="2"><?= GetMessage("BITRIX_MPBUILDER_KOMPONENTY_MODULA") ?></td>
	</tr>
	<tr>
		<td valign="top"><?= GetMessage("BITRIX_MPBUILDER_VYBERITE_KOMPONENTY") ?><br>(<?= GetMessage("BITRIX_MPBUILDER_IZ_PAPKI") ?> /bitrix/components <?= GetMessage("XTT_MPBUILDER_OR") ?> /local/components <?= GetMessage("BITRIX_MPBUILDER_KROME_SISTEMNYH_V") ?> bitrix)</td>
		<td>
			<?
			$strSelect = '';
			$strSelect .= '<select name="component[]" onchange="NewComponent(this)"><option value="" disabled selected>Select component</option>';
			$arComponents = [];

			function loadComponents($rootDir, $type)
			{
				global $arComponents;
				if (!is_dir($_SERVER['DOCUMENT_ROOT'] . $rootDir)) return false;
				$dir0 = opendir($path = $_SERVER['DOCUMENT_ROOT'] . $rootDir);
					...
					if (is_dir($path . '/' . $item0)) {
						...
						while (false !== $item1 = readdir($dir1)) {
							...
							if (is_dir($path . ($f = '/' . $item0 . '/' . $item1)))
								$arComponents[$rootDir . $f] = ['name' => $item0 . ':' . $item1, 'type' => $type];
						}
						closedir($dir1);
					}
				}
			...
			loadComponents(BX_ROOT . '/components', 'bitrix');
			loadComponents('/local/components', 'local');

			asort($arComponents);
			foreach ($arComponents as $key => $val)
				$strSelect .= '<option value="' . htmlspecialcharsbx($key) . '">' . htmlspecialcharsbx($val['name']) . ' (' . $val['type'] . ')</option>';
			$strSelect .= '</select>';
			?>
			<table id="componentlist" border="0" cellspacing="2" cellpadding="2">
				...
				if (is_array($_REQUEST['component'])) {
					foreach ($_REQUEST['component'] as $c) {
						if (!$c)
							continue;
						echo '<tr><td><select name="component[]"><option value="" disabled>Select component</option>';
						foreach ($arComponents as $key => $val)
	...
	<tr>
		<td></td>
		<td class="smalltext"><?= GetMessage("BITRIX_MPBUILDER_POSLE_SOZDANIA_MODUL") ?></td>
	</tr>
	<? $editTab->Buttons(); ?>
